...
struct sec_debug_regs_access sec_debug_last_regs_access[NR_CPUS];
...
static void map_noncached_sched_log_buf(void)
{
	...
	lastaddr =(unsigned long)(&sec_debug_log) + sizeof(struct sched_log);
	if ((((unsigned long)(&sec_debug_log)) & 0xfff00000) != (lastaddr & 0xfff00000))
		slog_buf_iodesc[0].length = 0x200000;
...
EXPORT_SYMBOL(get_sec_debug_level);

...
static int exynos_power_up_cpu(unsigned int cpu)
{
	...
	printk(KERN_ERR "ARM_CORE1_CONFIGURATION=[0x%x]", __raw_readl(EXYNOS_PMUREG(0x2080)));
	printk(KERN_ERR "ARM_CORE1_STATUS=[0x%x]", __raw_readl(EXYNOS_PMUREG(0x2084)));
	printk(KERN_ERR "PMU_SPARE2=[0x%x]", __raw_readl(EXYNOS_PMUREG(0x0908)));

	val = __raw_readl(power_base + 0x4);
	if (!(val & EXYNOS_CORE_LOCAL_PWR_EN)) {
		/* clear spare2 flag  */
		__raw_writel(0, EXYNOS_PMUREG(0x0908));
 ...
 */
		...
		printk(KERN_ERR "========== already EXYNOS_CORE_LOCAL_PWR_EN ===========");
		printk(KERN_ERR "========== Set 1 to Spare2 register  ===========");
		__raw_writel(1, EXYNOS_PMUREG(0x0908));
	}

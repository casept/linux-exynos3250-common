...
#ifdef CONFIG_KEYBOARD_GPIO_SEC_PP_KEY
#define GPIO_CANCEL_BUTTON	EXYNOS3_GPX2(0)
#define GPIO_RMS_1_BUTTON	EXYNOS3_GPX3(2)
#define GPIO_RMS_2_BUTTON	EXYNOS3_GPX3(3)
#define GPIO_RMS_3_BUTTON	EXYNOS3_GPX3(4)
#endif
...
int atmel_power(void *ddata, bool on)
{
	static struct regulator *regulator_pwr;
	static struct regulator *regulator_vdd;
	static bool enabled;
	int ret = 0;

	if (enabled == on) {
		pr_err("atmel-ts : %s same state!", __func__);
		return 0;
	}
	...
	if (IS_ERR(regulator_pwr)) {
		pr_err("atmel-ts : %s regulator_pwr error!", __func__);
		return PTR_ERR(regulator_pwr);
	}
	if (IS_ERR(regulator_vdd)) {
		pr_err("atmel-ts : %s regulator_vdd error!", __func__);
		return PTR_ERR(regulator_vdd);
	}
	...
	if (regulator_is_enabled(regulator_pwr) == !!on &&
		...
		pr_info("atmel-ts : %s %s", __func__, !!on ? "ON" : "OFF");
		enabled = on;
	} else {
		pr_err("atmel-ts : regulator_is_enabled value error!");
		ret = -1;
	}
...
void __init atmel_tsp_set_platdata(struct mxt_platform_data *pdata)
{
	if (!pdata)
		pdata = &atmel_ts_pdata;
...
//	pr_info("melfas-ts : W TSP init() is called : [%d]", system_rev);
	...
	gpio = atmel_ts_pdata.gpio_irq;
	ret = gpio_request(gpio, "TSP_INT");
	if (ret)
		pr_err("%s: failed to request TSP_INT gpio", __func__);
...
#ifdef CONFIG_KEYBOARD_GPIO_SEC_PP_KEY
	s3c_gpio_cfgpin(GPIO_CANCEL_BUTTON, S3C_GPIO_SFN(0xf));
	s3c_gpio_cfgpin(GPIO_RMS_1_BUTTON, S3C_GPIO_SFN(0xf));
	s3c_gpio_cfgpin(GPIO_RMS_2_BUTTON, S3C_GPIO_SFN(0xf));
	s3c_gpio_cfgpin(GPIO_RMS_3_BUTTON, S3C_GPIO_SFN(0xf));
#endif
...
#ifdef CONFIG_KEYBOARD_GPIO_SEC_PP_KEY
	s3c_gpio_setpull(GPIO_CANCEL_BUTTON, S3C_GPIO_PULL_DOWN);
	s3c_gpio_setpull(GPIO_RMS_1_BUTTON, S3C_GPIO_PULL_DOWN);
	s3c_gpio_setpull(GPIO_RMS_2_BUTTON, S3C_GPIO_PULL_DOWN);
	s3c_gpio_setpull(GPIO_RMS_3_BUTTON, S3C_GPIO_PULL_DOWN);
#endif
...
#ifdef CONFIG_KEYBOARD_GPIO_SEC_PP_KEY
	irq = s5p_register_gpio_interrupt(GPIO_CANCEL_BUTTON);
	if (IS_ERR_VALUE(irq)) {
		pr_err("%s: Failed to configure CANCEL GPIO\n", __func__);
		return;
	}
	irq = s5p_register_gpio_interrupt(GPIO_RMS_1_BUTTON);
	if (IS_ERR_VALUE(irq)) {
		pr_err("%s: Failed to configure RMS_1 GPIO\n", __func__);
		return;
	}
	irq = s5p_register_gpio_interrupt(GPIO_RMS_2_BUTTON);
	if (IS_ERR_VALUE(irq)) {
		pr_err("%s: Failed to configure RMS_2 GPIO\n", __func__);
		return;
	}
	irq = s5p_register_gpio_interrupt(GPIO_RMS_3_BUTTON);
	if (IS_ERR_VALUE(irq)) {
		pr_err("%s: Failed to configure RMS_3 GPIO\n", __func__);
		return;
	}
#endif
}

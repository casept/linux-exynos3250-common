...
#include "board-blink-gpio.h"
#elif defined CONFIG_MACH_SLIM
#include "board-slim-gpio.h"
#elif defined CONFIG_MACH_WC1
#include "board-orbis-gpio.h"
#elif defined CONFIG_MACH_HWP
#include "board-hwp-gpio.h"
#elif defined CONFIG_MACH_SMARTKEY
...
#else
	sleep_gpio_alive_table =sleep_gpio_table_alive[0];
	sleep_gpio_alive_table_size= INTERRUPT_SLEEP_GPIO_COUNT;
#endif /*end of model*/
}
...
#else
	init_gpio_alive_table = init_gpio_table_alive[0];
	init_gpio_alive_table_size = INTERRUPT_INIT_GPIO_COUNT;
#endif /*end of model*/
}
...
#else
	sleep_gpio_pd_table = sleep_gpio_table_pd[0];
	sleep_gpio_pd_table_size = AP_PD_GPIO_COUNT;
#endif /*end of model*/
}
...
#else
	init_gpio_tables = init_gpio_table[0];
	init_gpio_tables_size = AP_INIT_GPIO_COUNT;
#endif /*end of model*/
}
...
static void config_init_gpio_table_alive(void)
{
	...
	for (i = 0; i < OVERWRITE_INIT_ALIVE_COUNT; i++) {
		if (system_rev >= overwrite_init_alive_table[i][0]) {
			gpio = overwrite_init_alive_table[i][1];
			s3c_gpio_cfgpin(gpio, S3C_GPIO_SPECIAL(overwrite_init_alive_table[i][2]));
			s3c_gpio_setpull(gpio, overwrite_init_alive_table[i][3]);
		}
	}
}
...
static void config_init_gpio_table(void)
{
	...
	for (i = 0; i < OVERWRITE_INIT_GPIO_COUNT; i++) {
		if (system_rev >= overwrite_init_gpio_table[i][0]) {
			gpio = overwrite_init_gpio_table[i][1];
			s3c_gpio_cfgpin(gpio, overwrite_init_gpio_table[i][2]);
			s3c_gpio_setpull(gpio, overwrite_init_gpio_table[i][3]);
		}
	}
}
...
static void config_sleep_gpio_table_alive(void)
{
	...
	for (i = 0; i < OVERWRITE_SLEEP_ALIVE_COUNT; i++) {
		if (system_rev >= overwrite_sleep_alive_table[i][0]) {
			gpio = overwrite_sleep_alive_table[i][1];
			s3c_gpio_cfgpin(gpio, S3C_GPIO_SPECIAL(overwrite_sleep_alive_table[i][2]));
			s3c_gpio_setpull(gpio, overwrite_sleep_alive_table[i][3]);
		}
	}
}
...
static void config_sleep_gpio_table_pd(void)
{
	...
	for (i = 0; i < OVERWRITE_SLEEP_GPIO_COUNT; i++) {
		if (system_rev >= overwrite_sleep_gpio_table[i][0]) {
			gpio = overwrite_sleep_gpio_table[i][1];
			s5p_gpio_set_pd_cfg(gpio, overwrite_sleep_gpio_table[i][2]);
			s5p_gpio_set_pd_pull(gpio, overwrite_sleep_gpio_table[i][3]);
		}
	}
}
#endif

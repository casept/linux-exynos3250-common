	...
	if (charger->cable_type == POWER_SUPPLY_TYPE_BATTERY) {
		...
		if (charger->pdata->chg_float_voltage > charger->voltage_max) {
			data |= max14577_get_float_voltage_data(
				charger->voltage_max);
			dev_info(&charger->client->dev, "%s : float voltage in voltage_max (%dmV)\n",
		...
		if (batt_booting_chk) {
			...
			dev_info(&charger->client->dev, "%s : charging current(%dmA)data[0x%x]\n",
				__func__, charger->charging_current, data);
		...
		struct sec_charger_info *charger, int charging_current)
{
	...
	max14577_set_command(charger, MAX14577_CHG_REG_CHG_CTRL4, data);
	pr_info("%s: charging current[%d:0x%x]\n", __func__,
		charging_current, data);
	dump_stack();
}

static void max14577_set_float_voltage(
		struct sec_charger_info *charger, int float_vol)
{
	u8 data;

	pr_info("%s: float_vol[%d\n", __func__, float_vol);
	/* Battery-Charger Constant Voltage(CV) Mode, float voltage */
	data = 0;
	data |= max14577_get_float_voltage_data(float_vol);
	max14577_set_command(charger, MAX14577_CHG_REG_CHG_CTRL3, data);
}
			      ...
			      const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		...
		max14577_set_float_voltage(charger, val->intval);
		break;
	default:
		return false;
	}

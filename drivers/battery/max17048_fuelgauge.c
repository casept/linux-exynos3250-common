...
static void fg_test_read_regs(struct i2c_client *client)
{
	int data = 0;
	u32 addr = 0;
	...
	for (addr = 0x02; addr <= 0x04; addr += 2) {
		data = max17048_read_word(client, addr);
		pr_info("%s:0x%x[0x%04x], ", __func__, addr, data);
	}

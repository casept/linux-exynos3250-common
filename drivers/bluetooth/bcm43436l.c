...
static int bcm43436l_bt_rfkill_set_power(void *data, bool blocked)
{
...
static const struct rfkill_ops bcm43436l_bt_rfkill_ops = {
	.set_block = bcm43436l_bt_rfkill_set_power,
};
...
void bcm_bt_lpm_exit_lpm_locked(struct uart_port *uport)
{
	...
	if (bt_lpm.hci_registered) {
		/* 0.5 Sec */
		bt_lpm.enter_lpm_delay = ktime_set(0, NSEC_PER_SEC/2);
	} else {
		/* 1 sec */
		bt_lpm.enter_lpm_delay = ktime_set(0, NSEC_PER_SEC);
	}
...
static void update_host_wake_locked(int host_wake)
{
	...
	if (host_wake) {
		 ...
		 * more data to send.
		 */
		wake_lock_timeout(&bt_lpm.host_wake_lock, HZ/2); /* 500ms */

		pr_debug("[BT] ISR is low,unlock bt_is_running");
		bt_is_rx_running = 0;
		update_bt_is_running();
 ...
 */
static int hci_event(struct notifier_block *this,
					unsigned long event, void *data)
{
	...
	switch (event) {
		case HCI_DEV_REG:
			BT_INFO("[BT]HCI_DEV_REG Event Notify");

			bt_lpm.hci_registered = true;
			break;
		case HCI_DEV_UNREG:
			BT_INFO("[BT]HCI_DEV_UNREG Event Notify");
			bt_lpm.hci_registered = false;
			break;
...
static int bcm43436l_bluetooth_probe(struct platform_device *pdev)
{
	int rc = 0;
#ifdef BT_LPM_ENABLE
	int ret;
#endif
	rc = gpio_request(GPIO_BT_EN, "bcm43436l_bten_gpio");
	if (unlikely(rc)) {
		pr_err("[BT] GPIO_BT_EN request failed.\n");
		return rc;
	}
	rc = gpio_request(GPIO_BT_WAKE, "bcm43436l_btwake_gpio");
	if (unlikely(rc)) {
		pr_err("[BT] GPIO_BT_WAKE request failed.\n");
		gpio_free(GPIO_BT_EN);
		return rc;
	}
	rc = gpio_request(GPIO_BT_HOST_WAKE, "bcm43436l_bthostwake_gpio");
	if (unlikely(rc)) {
	...
	bt_rfkill = rfkill_alloc("bcm43436l Bluetooth", &pdev->dev,
				RFKILL_TYPE_BLUETOOTH, &bcm43436l_bt_rfkill_ops,
				NULL);
...
static int bcm43436l_bluetooth_remove(struct platform_device *pdev)
{
	rfkill_unregister(bt_rfkill);
	rfkill_destroy(bt_rfkill);
...
static struct platform_driver bcm43436l_bluetooth_platform_driver = {
	.probe = bcm43436l_bluetooth_probe,
	.remove = bcm43436l_bluetooth_remove,
	.driver = {
		   .name = "bcm43436l_bluetooth",
		   .owner = THIS_MODULE,
		   },
};

static int __init bcm43436l_bluetooth_init(void)
{
	return platform_driver_register(&bcm43436l_bluetooth_platform_driver);
}

static void __exit bcm43436l_bluetooth_exit(void)
{
	platform_driver_unregister(&bcm43436l_bluetooth_platform_driver);
	hci_unregister_notifier(&hci_event_nblock);
}


module_init(bcm43436l_bluetooth_init);
module_exit(bcm43436l_bluetooth_exit);

MODULE_ALIAS("platform:bcm43436l");
MODULE_DESCRIPTION("bcm43436l_bluetooth");
MODULE_LICENSE("GPL");

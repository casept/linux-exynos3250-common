	...
	char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (*ppos == 0) {
		dev = bus_find_device_by_name(&platform_bus_type, NULL, "secgpio_dvs");
...
static void  __exit gpio_dvs_plugin_exit(void)
{

}

module_init(gpio_dvs_plugin_init);
module_exit(gpio_dvs_plugin_exit);

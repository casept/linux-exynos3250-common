...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_TIZEN_B2
static
#endif
struct cpufreq_governor cpufreq_gov_tizen_b2 = {
       .name                   = "tizen_b2",
...
static struct workqueue_struct *tizen_b2_cpu_hotplug_workq;
...
extern unsigned int ssp_int_wakeup;
...
int needed_cpu_online(unsigned int cpu_load[], unsigned int cpu_freq_mhz)
{
	...
	case 1 :
		if ((exynos_check_enter_mode() == EXYNOS_CHECK_LPA)
			&& (cpu_freq_mhz <= 400)
			&& (cpu_load[0] < 90)) {
			need_cpu_online_num = 1;
		} else if ((cpu_freq_mhz >= 500)
			&& (cpu_load[0] >= 50)
			&& (get_run_task(5) >= 150)) {
	...
	/* There was some problem in wrist-up wakeup time.
	   So We are adding this patch for additional early up
	   of core */
	if (ssp_int_wakeup) {
		if (ssp_int_cnt < 6) {
			ssp_int_cnt++;
...
static void do_dbs_timer(struct work_struct *work)
{
	...
	queue_delayed_work_on(cpu, tizen_b2_cpu_hotplug_workq, &dbs_info->work, delay);
...
static inline void dbs_timer_init(struct cpu_dbs_info_s *dbs_info)
{
	...
	queue_delayed_work_on(dbs_info->cpu, tizen_b2_cpu_hotplug_workq,
			      &dbs_info->work, delay);
...
static struct workqueue_struct *cpu_exynos3250_wq;
struct delayed_work booting_cpu_boost_work;
static struct pm_qos_request pm_qos_cpu_online_min_req;

void exynos3250_cpu_boot_completed(struct work_struct *work)
{
...
static int __init cpufreq_gov_dbs_init(void)
{
	...
	tizen_b2_cpu_hotplug_workq = create_workqueue("cpu_hotplug");
	if (!tizen_b2_cpu_hotplug_workq) {
		pr_err("%s cannot create tizen_b2_cpu_hotplug_workq\n", __func__);
		goto err;
	}
	...
	cpu_exynos3250_wq = alloc_workqueue("cpu_exynos3250_wq", WQ_HIGHPRI, 0);
	if (!cpu_exynos3250_wq) {
		printk(KERN_ERR "Failed to create cpu_exynos3250_wq workqueue\n");
	}

	INIT_DELAYED_WORK(&booting_cpu_boost_work, exynos3250_cpu_boot_completed);

	queue_delayed_work(cpu_exynos3250_wq
				, &booting_cpu_boost_work, (CPU_BOOTING_TIME)*HZ);

	if (!pm_qos_request_active(&pm_qos_cpu_online_min_req)) {
		pm_qos_add_request(&pm_qos_cpu_online_min_req
					, PM_QOS_CPU_ONLINE_MIN, 2);
	}

	return cpufreq_register_governor(&cpufreq_gov_tizen_b2);

err :
	destroy_workqueue(tizen_b2_cpu_hotplug_workq);
	return -1;
...
{
	cpufreq_unregister_governor(&cpufreq_gov_tizen_b2);
	destroy_workqueue(tizen_b2_cpu_hotplug_workq);

...
static DEVICE_ATTR(sec_powerkey_pressed, S_IRUGO, key_pressed_show, NULL);
static DEVICE_ATTR(wakeup_keys, S_IWGRP, NULL, wakeup_enable);
...
#ifdef HARD_KEY_WAKEUP_BOOSTER
	if ((button->code == KEY_POWER) && (state != 0)) {
		hard_key_wakeup_booster_turn_on();
	}
#endif


	dev_info(&input->dev, "%s: [%s]%s[%s]\n", __func__,	(!!state) ? "P" : "R",
			button->desc ? button->desc : "UNKNOWN_KEY",
 ...
 */
static int power_keys_get_devtree_pdata(
		struct sec_pmic_dev *mfd_dev,
...
static int power_keys_probe(struct platform_device *pdev)
{
	...
	struct sec_pmic_platform_data *mpdata = dev_get_platdata(s2mpw01->dev);
	...
	if (!pdata) {
		ret = power_keys_get_devtree_pdata(s2mpw01, pdata);
		if (ret) {
			dev_err(dev, "failed to get devtree pdata\n");
	...
	sec_powerkey = sec_device_create(ddata, "sec_powerkey");
...
#endif
	dev_set_drvdata(sec_powerkey, ddata);
	ret = sysfs_create_group(&sec_powerkey->kobj, &sec_powerkey_attr_group);
	if (ret) {

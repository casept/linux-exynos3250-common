...
#include <linux/i2c/zxt_ztw522_ts_volt_pro.h>
...
retry:
	...
	if (ret < 0) {
		if (reg == ztw522_I2C_START_CMD)
			usleep_range(I2C_START_DELAY, I2C_START_DELAY+1);
		else
			zinitix_delay(I2C_TX_DELAY);
...
static irqreturn_t ztw522_touch_work(int irq, void *data)
{
	...
	if (!info->touch_info.status) {
		if ((!info->touch_info.finger_cnt) && (!info->touch_info.time_stamp)) {
			if (info->touch_info.coord[0].x == WET_MODE_VALUE)
				dev_info(&client->dev, "%s: wet mode\n", __func__);
			else if (info->touch_info.coord[0].x == NOR_MODE_VALUE)
				dev_info(&client->dev,
					"%s: normal mode\n", __func__);
			else
				dev_info(&client->dev,
					"%s: unkown mode(0x%04x)\n",
					__func__, info->touch_info.coord[0].x);
		} else
			dev_err(&client->dev,
				"%s: maybe periodical repeated interrupt\n",
				__func__);
		goto out;
	}
	...
	for (i = 0; i < info->cap_info.multi_fingers; i++) {
		...
		wet_state = info->touch_info.time_stamp;
		...
		if (zinitix_bit_test(sub_status, SUB_BIT_EXIST)) {
			...
			if (zinitix_bit_test(sub_status, SUB_BIT_DOWN)) {
				...
				dev_info(&client->dev, "%s: [%d][P] x=%3d, y=%3d, w=%d\n",
						__func__, i, x, y, wet_state);
#else
				dev_info(&client->dev,
						"%s: [%d][P] w = %d\n",
						__func__, i, wet_state);
#endif
...
#if !defined(CONFIG_SAMSUNG_PRODUCT_SHIP)
			dev_info(&client->dev, "%s: [%d][R] x=%3d, y=%3d,  dx=%3d, dy=%3d, dt=%d, m=%d, v=0x%02x%02x, md=%d\n",
				__func__, i, info->touch_info.coord[i].x, info->touch_info.coord[i].y,
				(info->touch_info.coord[i].x-info->event[i].x), (info->touch_info.coord[i].y-info->event[i].y),
				(int)(ktime_to_ms(ktime_get())-info->event[i].time),
				info->event[i].move_cnt, info->cap_info.fw_minor_version,
				info->cap_info.reg_data_version, info->optional_mode);
#else
				dev_info(&client->dev, "%s: [%d][R]\n", __func__, i);
#endif
			info->event[i].move_cnt = 0;
...
retry:
	...
	ret = ztw522_write_reg(client, ztw522_I2C_MODE_REG, REG_SET_I2C_LPM);
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev,
			"%s: Failed to read ztw522_I2C_MODE_REG. retry:[%d]\n", __func__, retry_cnt);
		if (retry_cnt < I2C_RETRY_TIMES) {
			retry_cnt++;
			goto retry;
		} else
...
retry:
	...
	ret = ztw522_write_reg(client, ztw522_I2C_MODE_REG, REG_SET_I2C_NORMAL);
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev,
			"%s: Failed to read ztw522_I2C_MODE_REG. retry:[%d]\n", __func__, retry_cnt);
		if (retry_cnt < I2C_RETRY_TIMES) {
...
retry:
	...
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev,
			"%s: Failed to write INTERNAL_FLAG_01. retry:[%d]\n", __func__, retry_cnt);
		if (retry_cnt < I2C_RETRY_TIMES) {
			retry_cnt++;
			goto retry;
		} else
	...
	retry_cnt = 0;
restore:
	/* restore INTERNAL_FLAG_01 & 02 */
	ret = ztw522_write_reg(client, ZINITIX_INTERNAL_FLAG_01, INTERNAL_FLAG_01_DEFAULT);
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev,
			"%s: Failed to write INTERNAL_FLAG_01. retry:[%d]\n", __func__, retry_cnt);
		if (retry_cnt < I2C_RETRY_TIMES) {
	...
	ret = ztw522_write_reg(client, ZINITIX_INTERNAL_FLAG_02, INTERNAL_FLAG_02_DEFAULT);
	if (ret != I2C_SUCCESS) {
		dev_err(&client->dev,
			"%s: Failed to write INTERNAL_FLAG_02. retry:[%d]\n", __func__, retry_cnt);
		if (retry_cnt < I2C_RETRY_TIMES) {
			retry_cnt++;
			goto restore;
		} else

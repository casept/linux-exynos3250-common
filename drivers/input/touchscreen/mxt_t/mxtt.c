...
static int mxt_input_open(struct input_dev *dev)
{
	...
	if (!data->probe_done) {
		printk(KERN_ALERT "%s: device is not initialised.\n", __func__);
		return 0;
	}
...
static int  mxt_touch_load_firmware(struct mxt_data *data)
{
	struct i2c_client *client = data->client;
		...
		const struct i2c_device_id *id)
{
	...
	error = mxt_touch_load_firmware(data);
	if (error) {
		tsp_debug_err(true, &client->dev, "Failed to load firmware\n");

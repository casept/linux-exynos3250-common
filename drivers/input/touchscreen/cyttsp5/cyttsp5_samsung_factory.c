...
static const XY excluded_nodes[] =
{
	{0,0},{0,1},{1,0},
	{0,5},{0,6},{1,6},
	{5,0},{6,0},{6,1},
...
static bool node_excluded(u8 x, u8 y)
{
	int i;
	for (i = 0; i < ARRAY_SIZE(excluded_nodes); i++)
	{
		if (x == excluded_nodes[i].x &&
			y == excluded_nodes[i].y)
			return true;
	}
	return false;
}
	...
	u8 *buf, int num_nodes, s16 *max_value,	s16 *min_value)
{
	...
	*min_value = 0x7FFF;
	...
	for (i = 0; i < num_nodes; i++) {
		if ( node_excluded(i/NUM_Y, i%NUM_Y) ) {
		...
		*devattr, const char *buf, size_t count)
{
	...
	if (ambient_mode) {
		...
		if (cd->trustedUI_mode) {
			cd->ambient_flag = true;
			dev_info(dev, "%s: TRUSTONIC_TRUSTED_UI is enabled.\n", __func__);
			return count;
		}
#endif
		ret = cyttsp5_core_ambient_on(sfd->dev);
		if (ret) {
			dev_err(sfd->dev, "%s: Ambient mode on failed.\n", __func__);
			cyttsp5_core_start(sfd->dev);
			cyttsp5_core_ambient_on(sfd->dev);
		}
	} else {
#ifdef CONFIG_TRUSTONIC_TRUSTED_UI_NOTIFY
		cd->ambient_flag = false;
#endif
		ret = cyttsp5_core_ambient_off(sfd->dev);
		if (ret) {
			dev_err(sfd->dev, "%s: Ambient mode off failed.\n", __func__);
			cyttsp5_core_start(sfd->dev);
		}
	}

...
void cyttsp5_mt_lift_all(struct cyttsp5_mt_data *md)
{
	...
	if (md->num_prv_tch != 0) {
		...
		input_sync(md->input);
#ifdef TOUCH_BOOSTER
		/* set_dvfs_lock(md, TOUCH_BOOSTER_QUICK_OFF); */
	...
	struct cyttsp5_touch *touch, u8 *xy_data)
{
	...
	/*
	 * call exynos_smc_gearpay with for every touch event.
	 * g_lock_layout_active is set in the ioctl handler once Privacy
	 * Lock sends an event that it is active and it has drawn the
	 * Lock Screen layout.
	 */
...
#ifdef SAMSUNG_PALM_MOTION
	report_sumsize_palm(md, sumsize, md->largeobj);
#endif
...
static void cyttsp5_mt_send_dummy_event(struct cyttsp5_mt_data *md)
{
	...
	input_report_abs(md->input, ABS_MT_PALM, 0);
...
void cyttsp5_mt_run_close(struct device *dev)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
...
#ifdef CONFIG_TRUSTONIC_TRUSTED_UI_NOTIFY
	if (cd->trustedUI_mode) {
		cd->close_flag = true;
		dev_info(dev, "%s: TRUSTONIC_TRUSTED_UI is enabled.\n", __func__);
		return;
	}
#endif
	cyttsp5_mt_run_close(dev);
...
int cyttsp5_mt_probe(struct device *dev)
{
	...
	if ((cd->silicon_id != CSP_SILICON_ID) && (cd->silicon_id != BGA_SILICON_ID))
		mt_pdata->flags =CY_MT_FLAG_FLIP | CY_MT_FLAG_INV_X;

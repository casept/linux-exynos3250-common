...
static struct opp *exynos3250_int_saved_target_opp;
void exynos3250_devfreq_int_store_target_opp(struct opp *target_opp)
{
	exynos3250_int_saved_target_opp = target_opp;
}

struct opp * exynos3250_devfreq_int_get_target_opp(void)
{
	return exynos3250_int_saved_target_opp;
}
				      ...
				      u32 flags)
{
	...
	*target_freq = max((unsigned long)pm_qos_request(PM_QOS_DEVICE_THROUGHPUT), *target_freq);
	...
	exynos3250_devfreq_int_store_target_opp(target_opp);
...
#if defined(CONFIG_SLP_MINI_TRACER)
{
	...
	if (old_freq < *target_freq) {
		regulator_set_voltage(int_data->vdd_int, target_volt, REG_MAX_VOLT_LIMIT);
...
#if defined(CONFIG_SYSTEM_LOAD_ANALYZER)
	store_external_load_factor(INT_BUS_LOAD, (int_pmcnt * 100) /int_ccnt);
	store_external_load_factor(INT_BUS_FREQ, stat->current_frequency);
#endif
...
#if defined(CONFIG_EXYNOS_THERMAL) && !defined(CONFIG_MACH_BLINK)
						...
						void *v)
{
	...
	target_opp = exynos3250_devfreq_int_get_target_opp();
	...
	if (*on) {
		...
		regulator_set_voltage(data->vdd_int, (volt + volt_offset), REG_MAX_VOLT_LIMIT);
		pr_info("%s *on=%d volt=%d volt_offset=%d\n", __FUNCTION__, *on, volt, volt_offset);
	} else {
		if (!volt_offset)
			goto out;
		else
		...
		volt = opp_get_voltage(target_opp);
		regulator_set_voltage(data->vdd_int, volt, REG_MAX_VOLT_LIMIT);
		pr_info("%s *on=%d volt=%d volt_offset=%d\n", __FUNCTION__, *on, volt, volt_offset);
	}
...
static __devinit int exynos3250_busfreq_int_probe(struct platform_device *pdev)
{
	...
	queue_delayed_work(bus_int_wq, &int_fastboot_work, (45)*HZ);

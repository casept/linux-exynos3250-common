...
static int exynos3250_devfreq_set_dll_voltage(struct devfreq_data_mif *data, int target_idx)
{
	...
	regulator_set_voltage(data->vdd_mif, target_volt, REG_MAX_VOLT_LIMIT);
...
static struct opp *exynos3250_mif_saved_target_opp;
void exynos3250_devfreq_mif_store_target_opp(struct opp *target_opp)
{
	exynos3250_mif_saved_target_opp = target_opp;
}

struct opp * exynos3250_devfreq_mif_get_target_opp(void)
{
	return exynos3250_mif_saved_target_opp;
}
					    ...
					    u32 flags)
{
	...
	*target_freq = max((unsigned long)pm_qos_request(PM_QOS_BUS_THROUGHPUT), *target_freq);
	...
	exynos3250_devfreq_mif_store_target_opp(target_opp);
...
#if defined(CONFIG_SYSTEM_LOAD_ANALYZER)
	store_external_load_factor(MIF_BUS_LOAD, (mif_pmcnt * 100) /mif_ccnt);
	store_external_load_factor(MIF_BUS_FREQ, stat->current_frequency);
#endif
						...
						void *v)
{
	...
	target_opp = exynos3250_devfreq_mif_get_target_opp();
	...
	if (*on) {
		...
		regulator_set_voltage(data->vdd_mif, (volt + volt_offset), REG_MAX_VOLT_LIMIT);
		...
		volt = opp_get_voltage(target_opp);
		regulator_set_voltage(data->vdd_mif, volt, REG_MAX_VOLT_LIMIT);
		pr_info("%s *on=%d volt=%d volt_offset=%d\n", __FUNCTION__, *on, volt, volt_offset);
	}
...
static __devinit int exynos3250_busfreq_mif_probe(struct platform_device *pdev)
{
	...
	queue_delayed_work(bus_mif_wq, &mif_fastboot_work, (45)*HZ);

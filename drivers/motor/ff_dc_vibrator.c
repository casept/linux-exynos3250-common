...
static void vib_en(bool en)
{
	...
	if (en) {
		...
		err = regulator_set_voltage(g_hap_data->regulator,
...
static int ff_dc_vibrator_probe(struct platform_device *pdev)
{
	...
	ret = regulator_set_voltage(hap_data->regulator, hap_data->pdata->min_volt,
			hap_data->pdata->max_volt);
	if (ret) {
		pr_info("%s : regulator set_voltage fail\n", __func__);
...
static int __devexit ff_dc_vibrator_remove(struct platform_device *pdev)
{

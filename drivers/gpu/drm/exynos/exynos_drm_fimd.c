...
static void fimd_trigger(struct device *dev)
{
	...
	if (first_open) {
		...
		if (!fimd_check_event(ctx, __func__)) {
			DRM_INFO("%s:bypass\n", __func__);
			return;
		}
	}
...
static inline void fimd_win_blank_ctrl(struct fimd_context *ctx, int win, bool blank)
{
	...
	u32 cl_map = WINxMAP_MAP | WINxMAP_MAP_COLOUR(0x00);

	if (blank)
		writel(cl_map, ctx->regs + WINxMAP(win));
	else
...
static void fimd_wait_for_vblank(struct device *dev)
{
	...
	if (fimd_is_lpm(ctx)) {
		DRM_INFO("[wait_for_vbl]bypass:bl_off\n");
		return;
	}

	fimd_check_event(ctx, "wait_for_vbl");
			      ...
			      struct exynos_drm_overlay *overlay)
{
	...
	win_data->sync = overlay->sync;

	if (timing->refresh != win_data->refresh) {
		/* FIXME: fimd clock configure ? */
		DRM_INFO("%s:refresh[%d->%d]\n", __func__, timing->refresh, win_data->refresh);
		/* MIPI configuration */
		dsim = platform_get_drvdata(ctx->disp_bus_pdev);
...
static void fimd_win_commit(struct device *dev, int zpos)
{
	...
	DRM_DEBUG("%s:win[%d]a[0x%x]val[%d %d]dpms[%d]bl_on[%d]sync[%d]\n",
		"commit", win, readl(ctx->regs + VIDWx_BUF_START(win, 0)),
		atomic_read(&ctx->win_update), atomic_read(&ctx->do_apply),
		ctx->dpms, ctx->bl_on, win_data->sync);

	if (atomic_read(&ctx->win_update) && win_data->sync) {
		int ret;

		DRM_INFO("%s:win[%d]dpms[%d]bl_on[%d]sync:wait\n",
			"commit", win, ctx->dpms, ctx->bl_on);

		if (completion_done(&ctx->sync_comp))
			INIT_COMPLETION(ctx->sync_comp);

		ret = wait_for_completion_timeout(&ctx->sync_comp,
			msecs_to_jiffies(200));
		if (ret < 0)
			DRM_ERROR("sync_comp:ret[%d]", ret);
		else if (!ret)
			DRM_INFO("%s:timeout\n", "commit_wait");
		if (ret <= 0)
			complete_all(&ctx->sync_comp);

		DRM_INFO("%s:win[%d]dpms[%d]bl_on[%d]sync:done\n",
			"commit", win, ctx->dpms, ctx->bl_on);
	}
}

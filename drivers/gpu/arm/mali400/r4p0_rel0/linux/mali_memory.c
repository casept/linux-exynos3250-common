...
u32 _mali_kernel_memory_dump_state(char *buf, u32 size)
{
	...
	u32 mali_mem_usage, mali_mem_os;

	n += _mali_osk_snprintf(buf + n, size - n,
		"     pid\tmali_mem\texternal_mem\tdma_mem\n");
	n += _mali_osk_snprintf(buf + n, size - n,
		"==========================================================\n");
	...
	MALI_SESSION_FOREACH(session, tmp, link) {
		int i;
		u32 mem_os_block = 0, mem_external = 0, mem_dma_buf = 0;

		_mali_osk_mutex_wait(session->memory_lock);

		_mali_osk_mutex_rw_wait(session->descriptor_mapping->lock,
					_MALI_OSK_LOCKMODE_RO);

		/*
		 * id 0 is skipped as it's an reserved ID
		 * not mapping to anything
		 */
		for (i = 1;
			i < session->descriptor_mapping->current_nr_mappings;
			i++) {
			if (_mali_osk_test_bit(i,
				session->descriptor_mapping->table->usage)) {
				mali_mem_allocation *descriptor =
					(mali_mem_allocation *)
				session->descriptor_mapping->table->mappings[i];
				switch (descriptor->type) {
				case MALI_MEM_OS:
				case MALI_MEM_BLOCK:
					mem_os_block += descriptor->size;
					break;
				case MALI_MEM_EXTERNAL:
					mem_external += descriptor->size;
					break;
				case MALI_MEM_DMA_BUF:
					mem_dma_buf += descriptor->size;
					break;
				default:
					break;
				}
		...
		_mali_osk_mutex_rw_signal(session->descriptor_mapping->lock,
					_MALI_OSK_LOCKMODE_RO);
		...
		n += _mali_osk_snprintf(buf + n, size - n,
					"%8d\t%8u\t%8u\t%8u\n",
					session->pid, mem_os_block,
					mem_external, mem_dma_buf);
	}
	mali_session_unlock();

 ...
 * ver   who                                         what                      *
 * ---- -------------------------------------------- ------------------------- *
 * 001   Sangin Lee <sangin78.lee@samsung.com>       Initial Release           *
 *       Sanghyeon Lee <sirano06.lee@samsung.com>                              *
 *       Junyoun Kim <junyouns.kim@samsung.com>                                *
 * ---- -------------------------------------------- ------------------------- *
 * 002   Hunsup Jung <hunsup.jung@samsung.com>       Remove unnecessary code   *
...
// TODO: Need to seperate model specific information
#if defined(CONFIG_MACH_VOLT) || defined(CONFIG_MACH_VOLT_NE)
// FIXME:: Need to update with volt configuration
static struct energy_mon_mapping_table_t energy_mon_table[] = {
	/* ENERGY_MON_WAKEUP_INPUT */
	{299, ENERGY_MON_WAKEUP_INPUT},	/* sec_touchscreen */
	{315, ENERGY_MON_WAKEUP_INPUT},	/* KEY_BACK */
	{421, ENERGY_MON_WAKEUP_INPUT}, /* pwronr-irq */
	{422, ENERGY_MON_WAKEUP_INPUT}, /* pwronf-irq */

	/* ENERGY_MON_WAKEUP_SSP */
	{290, ENERGY_MON_WAKEUP_SSP}, /* ttyBCM */

	/* ENERGY_MON_WAKEUP_RTC */
	{431, ENERGY_MON_WAKEUP_RTC}, /* rtc-alarm0 */

	/* ENERGY_MON_WAKEUP_BT */
	{310, ENERGY_MON_WAKEUP_BT}, /* bt host_wake */

	/* ENERGY_MON_WAKEUP_WIFI */
	{297, ENERGY_MON_WAKEUP_WIFI}, /* bcmsdh_sdmmc */

	/* ENERGY_MON_WAKEUP_CP */

	/* Last entry. Don't move */
	{-1, -1}
};
#else
static struct energy_mon_mapping_table_t energy_mon_table[] = {
	/* ENERGY_MON_WAKEUP_INPUT */
	{289, ENERGY_MON_WAKEUP_INPUT},
	{296, ENERGY_MON_WAKEUP_INPUT},
	{303, ENERGY_MON_WAKEUP_INPUT},
	{299, ENERGY_MON_WAKEUP_INPUT},
	{315, ENERGY_MON_WAKEUP_INPUT},
	{311, ENERGY_MON_WAKEUP_INPUT},

	/* ENERGY_MON_WAKEUP_SSP */
	{290, ENERGY_MON_WAKEUP_SSP},

	/* ENERGY_MON_WAKEUP_RTC */
	{295, ENERGY_MON_WAKEUP_RTC},

	/* ENERGY_MON_WAKEUP_BT */
	{310, ENERGY_MON_WAKEUP_BT},

	/* ENERGY_MON_WAKEUP_WIFI */
	{297, ENERGY_MON_WAKEUP_WIFI},
...
static void energy_mon_get_additional_info(struct energy_mon_data *p_curr, struct energy_mon_data *p_prev)
{
...
{
	struct energy_mon_data *p_buff = NULL;
	struct timespec time_diff;
	int charging_discharging = 0;
	int i;
	...
	if (type == ENERGY_MON_TYPE_BATTERY || type == ENERGY_MON_TYPE_MONITOR) {
		...
		if (p_prev->bat_status == POWER_SUPPLY_STATUS_DISCHARGING) {
			p_buff = &energy_mon.discharging;
			energy_mon.discharging_count++;
			charging_discharging = ENERGY_MON_STATE_DISCHARGING;
		} else if (p_prev->bat_status == POWER_SUPPLY_STATUS_CHARGING) {
...
int energy_monitor_marker(int type)
{
	...
	energy_mon_dbg(ENERGY_MON_DEBUG_DBG,
			"sizeof(energy_mon.wakeup_cause)=%d\n",
			sizeof(energy_mon.wakeup_cause));

	/* Get additional informations */
	if (type == ENERGY_MON_TYPE_BATTERY || type == ENERGY_MON_TYPE_MONITOR)
		energy_mon_get_additional_info(p_curr, p_prev);
...
static int energy_monitor_find_wakeup_index(int irq)
{
	int i;

	for (i = 0; ; i++) {
		if (irq == energy_mon_table[i].irq)
			return energy_mon_table[i].wakeup_idx;
		if (energy_mon_table[i].irq == -1)
			return -ENXIO;
	}
}

int energy_monitor_record_wakeup_reason(int irq)
{
	...
	wakeup_idx = energy_monitor_find_wakeup_index(irq);
...
static ssize_t energy_monitor_print_time_logs(char *buf, int sizeof_buf, int type)
{
	...
	ret += snprintf(buf + ret, sizeof_buf - ret,
...
static ssize_t energy_mon_summary_print(char *buf, int sizeof_buf,
		ssize_t ret, enum energy_mon_print_type p_type)
{
	...
	if (p_type == ENERGY_MON_PRINT_TITLE) {
		...
		ret += snprintf(buf + ret, sizeof_buf - ret, "[summary]\n");
		ret += snprintf(buf + ret, sizeof_buf - ret,
				...
				"CUR_AVER_/KERN_%%/DISP_%%/INP/SSP/RTC/BT_/WIF/CP_\n");
		ret += energy_monitor_print_time_logs(buf + ret, sizeof_buf, ENERGY_MON_TYPE_BOOTING);
	} else if (p_type == ENERGY_MON_PRINT_MAIN)
		ret += energy_monitor_print_time_logs(buf + ret, sizeof_buf, ENERGY_MON_TYPE_BATTERY);
	else {
		ret += energy_monitor_print_time_logs(buf + ret, sizeof_buf, ENERGY_MON_TYPE_DUMP);
		ret += snprintf(buf + ret, sizeof_buf - ret, "\n");
		ret += energy_monitor_print_time_logs(buf + ret, sizeof_buf, ENERGY_MON_TYPE_CHARGING);
		ret += energy_monitor_print_time_logs(buf + ret, sizeof_buf, ENERGY_MON_TYPE_DISCHARGING);
		ret += snprintf(buf + ret, sizeof_buf - ret, "\n");
	}
...
#ifdef CONFIG_SENSORHUB_STAT
static ssize_t energy_monitor_print_sensorhub_logs(char *buf, int sizeof_buf,
		enum energy_mon_print_type p_type)
{
	...
	energy_mon_dbg(ENERGY_MON_DEBUG_DBG, "%s: sizeof_buf=%d, p_type=%d\n"
		, __func__, sizeof_buf, p_type);
	...
	if (p_type == ENERGY_MON_PRINT_TITLE) {
		...
		ret += snprintf(buf + ret, sizeof_buf - ret, "[sensorhub]\n%c/idx%s\n"
				, print_type, temp_buf);
	}
	energy_mon_dbg(ENERGY_MON_DEBUG_DBG, "%s: p_curr=%p, p_prev=%p\n", __func__, p_curr, p_prev);
	...
	ret += snprintf(buf + ret, sizeof_buf - ret, "%c/%03d%s\n"
			, print_type, p_curr->log_count
			, temp_buf);
...
#ifdef CONFIG_SAP_PID_STAT
static ssize_t energy_monitor_print_sap_logs(char *buf, int sizeof_buf,
	enum energy_mon_print_type p_type)
{
	ssize_t ret = 0, ret_temp = 0;
	struct energy_mon_data *p_curr = NULL;
	...
	if (p_type == ENERGY_MON_PRINT_TITLE) {
		...
		ret += snprintf(buf + ret, sizeof_buf - ret, "[sap]\n%c/idx%s\n"
				, print_type, temp_buf);
	}
...
#ifdef CONFIG_SLAVE_WAKELOCK
static ssize_t energy_monitor_print_slave_wakelock_logs(char *buf, int sizeof_buf,
		enum energy_mon_print_type p_type)
{
	ssize_t ret = 0, ret_temp = 0;
	struct energy_mon_data *p_curr = NULL;
	...
	if (p_type == ENERGY_MON_PRINT_TITLE) {
		...
		ret += snprintf(buf + ret, sizeof_buf - ret, "[slave_wakelock]\n%c/idx%s\n"
				, print_type, temp_buf);
...
static ssize_t energy_monitor_print_gps_logs(char *buf, int sizeof_buf,
		enum energy_mon_print_type p_type)
{
	ssize_t ret = 0;
	struct energy_mon_data *p_curr = NULL;
	...
	/* Assign proper buffer to use */
	if (p_type == ENERGY_MON_PRINT_TITLE) {
		ret += snprintf(buf + ret, sizeof_buf - ret, "[gps]\n");
		ret += snprintf(buf + ret, sizeof_buf - ret, "%c/idx/time/", print_type);
		ret += snprintf(buf + ret, sizeof_buf - ret, "___last_gps_user/ext/\n");
	}
	...
	ret += snprintf(buf + ret, sizeof_buf - ret, "%c/%03d/%04d/", print_type, p_curr->log_count, gps_time);
	ret += snprintf(buf + ret, sizeof_buf - ret, "%016llx/", p_curr->sh_gps.last_gps_user);
	ret += snprintf(buf + ret, sizeof_buf - ret, " %02u/\n", p_curr->sh_gps.last_gps_ext);
...
static ssize_t energy_mon_print(char *buf, int sizeof_buf, ssize_t ret,
		enum energy_mon_print_step print_step, enum energy_mon_print_type p_type)
{
	switch (print_step) {
	case STEP_SUMMARY:
		ret += energy_mon_summary_print(buf, sizeof_buf, ret, p_type);
		break;
#ifdef CONFIG_SENSORHUB_STAT
	case STEP_SENSORHUB_STAT:
		ret += energy_monitor_print_sensorhub_logs(buf + ret, sizeof_buf, p_type);
		break;
#endif
#ifdef CONFIG_SAP_PID_STAT
	case STEP_SAP_STAT:
		ret += energy_monitor_print_sap_logs(buf + ret, sizeof_buf, p_type);
		break;
#endif
#ifdef CONFIG_SLAVE_WAKELOCK
	case STEP_SLAVE_WAKELOCK:
		ret += energy_monitor_print_slave_wakelock_logs(buf + ret, sizeof_buf, p_type);
		break;
#endif
#ifdef CONFIG_SENSORHUB_STAT
	case STEP_GPS_STAT:
		ret += energy_monitor_print_gps_logs(buf + ret, sizeof_buf, p_type);
		break;
#endif
	default:
		break;
...
static int energy_monitor_init(void)
{
	...
	energy_mon_dbg(ENERGY_MON_DEBUG_DBG,
			"%s: sizeof(struct energy_mon_data) is %d\n",
			__func__, sizeof(struct energy_mon_data));
	energy_mon_dbg(ENERGY_MON_DEBUG_DBG,
			"%s: sizeof(energy_mon) is %d(ENERGY_MON_HISTORY_NUM=%d)\n",

...
#define MIN_SLEEP_TIME_S 1000 /* second */
...
#endif
#ifdef CONFIG_SLEEP_MONITOR_SLAVE_WAKELOCK
       struct slp_mon_slave_wakelocks slwl[SLEEP_MON_SLWL_ARRAY_SIZE];
#endif
#ifdef CONFIG_SENSORHUB_STAT
...
int energy_monitor_find_wakeup_index(int irq)
{
	int i;
...
int energy_monitor_marker(int type, int state)
{
	...
	energy_mon_dbg(ENERGY_MON_DEBUG_DBG, "sizeof(energy_mon.wakeup_cause)=%d\n", sizeof(energy_mon.wakeup_cause));
...
#endif
#ifdef CONFIG_SLEEP_MONITOR_SLAVE_WAKELOCK
               get_sleep_monitor_slave_wakelock(p_curr->slwl, SLEEP_MON_SLWL_ARRAY_SIZE);
#endif
#ifdef CONFIG_SENSORHUB_STAT
...
static ssize_t energy_monitor_print_time_logs(char *buf, int sizeof_buf, int type)
{
	...
	rtc_time_to_tm(p_curr->ts_real.tv_sec, &tm_real);

	ret += snprintf(buf + ret, sizeof_buf, "%c/%03d/%d/%d/%6d/%04d/%10lu.%03lu/%04d-%02d-%02d %02d:%02d:%02d/%6lu.%03lu/%6lu.%03lu/%6lu.%03lu%s"
...
enum energy_mon_print_step {
	...
	if (*ppos == 0) {
		...
		ret += snprintf(buf + ret, sizeof(buf),
			"T/CNT/S/B/CAPA__/SUSP/REAL_TIME_UTC_/REAL_TIME_RTC_(UTC)/BOOT_TIME_/KERN_TIME_/DISP_TIME_/dSOC__/dBOOT/dKERN/dDISP/CUR_AVER/KERN_%%/DISP_%%/INP/SSP/RTC/BT_/WIF/CP_\n");
		...
		case PM_SUSPEND_PREPARE: {
			if (energy_mon.last_wakeup < ENERGY_MON_WAKEUP_MAX && energy_mon.last_wakeup >= 0) {
				...
				if (energy_mon.last_wakeup == ENERGY_MON_WAKEUP_SSP && diff_time.tv_sec >= 0) {
					if (diff_time.tv_sec > ENERGY_MON_MAX_WAKEUP_STAT_TIME)
						diff_time.tv_sec = ENERGY_MON_MAX_WAKEUP_STAT_TIME;

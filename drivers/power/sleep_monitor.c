...
static sleep_monitor_device slp_mon[SLEEP_MONITOR_NUM_MAX] = {
	...
	{NULL, NULL, SLEEP_MONITOR_CHECK_HARD, SLEEP_MONITOR_BOOLEAN_FALSE,"DEV22   " }, /* SLEEP_MONITOR_DEV22 */
...
{
	const int SLEEP_MONITOR_LPM_MODE = 1;

	slp_mon_retry_cnt = 0;
	store_buf = (char *)kmalloc(sizeof(char*) * SLEEP_MONITOR_STORE_BUFF, GFP_KERNEL);
	if (!store_buf) {
	...
	if (slp_mon_d) {
		if (!debugfs_create_file("status", S_IRUSR
			, slp_mon_d, NULL, &sleep_monitor_debug_fops)) \
				pr_err("%s : debugfs_create_file, error\n", "status");

		if (!debugfs_create_file("skip_device", S_IRUSR | S_IWUSR
			, slp_mon_d, NULL, &skip_device_fops))  \
				pr_err("%s : debugfs_create_file, error\n", "skip_device");

		if (!debugfs_create_file("check_device_level", S_IRUSR | S_IWUSR
			, slp_mon_d, NULL, &device_level_fops))  \
				pr_err("%s : debugfs_create_file, error\n", "check_device_level");

		if (!debugfs_create_file("pretty_status", S_IRUSR
			, slp_mon_d, NULL, &device_pretty_fops))  \
				pr_err("%s : debugfs_create_file, error\n", "pretty_status");

		if (!debugfs_create_file("monitor_interval", S_IRUSR | S_IWUSR
			, slp_mon_d, NULL, &monitor_interval_fops))  \
				pr_err("%s : debugfs_create_file, error\n", "monitor_interval");

		if (!debugfs_create_file("dev_name", S_IRUGO
                        , slp_mon_d, NULL, &dev_name_fops))  \
                                pr_err("%s : debugfs_create_file, error\n", "dev_name");

		if (!debugfs_create_file("store_buf", S_IRUSR
                        , slp_mon_d, NULL, &store_buf_fops))  \
                        pr_err("%s : debugfs_create_file, error\n", "store_buf");

		if (!debugfs_create_file("last_marker", S_IWUSR
                        , slp_mon_d, NULL, &last_marker_fops))  \
                        pr_err("%s : debugfs_create_file, error\n", "last_marker");

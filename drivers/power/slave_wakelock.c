...
static const char internal_lock[30][20] = {
	"", /* 100000 */
	"i_alpm", "i_bat", "i_batfull", "i_power", "i_booting",     /* 100001~100005 */
	"i_dump", "i_hdmi", "i_popup", "i_standby", "i_taoff",      /* 100006~100010 */
	"i_ta", "i_usboff", "i_usb", "i_poweroff", "i_cooldown",    /* 100011~100015 */
	"i_lowbat", "i_lowmem", "i_esd", "i_hall", "i_cradle",      /* 100016~100020 */
	"i_mmc", "i_system", "i_telephony", "i_udevoff", "i_udev",  /* 100021~100025 */
	"i_lcdoff_req", "" /* 100026~100027 */
};
...
static int sort_energy_mon_time(struct slp_mon_slave_wakelocks *slwl, int size)
{
	struct slave_wakelock *iter;
	ktime_t ktime_zero = ktime_set(0, 0);
	int i = 0, j = 0;
	...
	rcu_read_lock();
	list_for_each_entry_rcu(iter, &slwl_list, entry) {
		if (ktime_to_ms(iter->energy_mon_time) <= 0)
			continue;
...
void get_sleep_monitor_slave_wakelock(struct slp_mon_slave_wakelocks *slwl, int size)
{
	...
	num = sort_energy_mon_time(slwl, size);
...
#ifdef CONFIG_SLEEP_MONITOR
	ret = seq_printf(m, "      %12lld", ktime_to_ms(slwl->slp_mon_time));
#endif
#ifdef CONFIG_ENERGY_MONITOR
	ret += seq_printf(m, "    %15lld %21lld",

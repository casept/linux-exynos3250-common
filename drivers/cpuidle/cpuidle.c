...
int cpuidle_idle_call(void)
{
	...
	cpuidle_force_set(&next_state, next_state);
#endif

#if defined(CONFIG_SLP_CPU_TESTER)
	if (dev->cpu == 0) {
		if (cpuidle_tester_en || cpu_idle_trace_en)
			cpu_test_idle_table[next_state].enter_count++;
	}
#endif
	...
	if (dev->cpu == 0) {
		if (cpuidle_tester_en || cpu_idle_trace_en)
			cpu_test_idle_table[next_state].exit_count++;
	}
#endif

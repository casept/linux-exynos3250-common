...
static int dhd_write_cid_file(const char *filepath_cid, const char *buf, int buf_len)
{
	...
	fp = filp_open(filepath_cid, O_RDWR | O_CREAT | O_SYNC, 0666);
	if (IS_ERR(fp)) {
		DHD_ERROR(("[WIFI_SEC] %s: File open error\n", filepath_cid));
		return -1;
	} else {
...
startwrite:
	...
	fp_mac = filp_open(filepath_data, O_RDWR | O_CREAT | O_SYNC, 0666);
	...
	fp_mac = filp_open(filepath_efs, O_RDWR | O_CREAT | O_SYNC, 0666);

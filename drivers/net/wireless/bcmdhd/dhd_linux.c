...
static int dhd_process_cid_mac(dhd_pub_t *dhdp, bool prepost)
{
	...
	char *filepath_efs	= "/csa/.mac.info";
...
#ifdef CONFIG_SYSTEM_LOAD_ANALYZER
	store_external_load_factor(CONN_WIFI_TX, skb->len);
#endif
...
#ifdef CONFIG_SYSTEM_LOAD_ANALYZER
		store_external_load_factor(CONN_WIFI_RX, skb->len);
#endif
...
#ifdef CONFIG_SYSTEM_LOAD_ANALYZER
	store_external_load_factor(CONN_WIFI_ENABLED, 0);
#endif
...
#ifdef CONFIG_SYSTEM_LOAD_ANALYZER
	store_external_load_factor(CONN_WIFI_ENABLED, 1);
#endif
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	{
		uint32 grat_arp = 1;
		bcm_mkiovar("grat_arp", (char *)&grat_arp, 4, iovbuf, sizeof(iovbuf));
		if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0)) < 0)
			DHD_ERROR(("%s wl grat_arp 1 failed %d\n", __FUNCTION__, ret));
	}
#endif
...
dhd_module_init(void)
{
	...
	INIT_DELAYED_WORK_DEFERRABLE(&init_work, dhd_wifi_platform_register_drv);
	err = schedule_delayed_work(&init_work, msecs_to_jiffies(1000));

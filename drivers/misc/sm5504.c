...
static const struct device_desc device_to_cable_type_mapping[] = {
	{
	 ...
	 .cable_type = MUIC_CABLE_TYPE_OTG,
	 },
	{
	 .name = "USB SDP",
	 .reg_val = SM5504_DEVICE1_SDP,
	 .cable_type = MUIC_CABLE_TYPE_USB,
	 },
	{
	 .name = "UART",
	 .reg_val = SM5504_DEVICE1_UART,
	 .cable_type = MUIC_CABLE_TYPE_UART,
	 },
	{
	 .name = "USB CDP",
	 .reg_val = SM5504_DEVICE1_CDPORT,
	 .cable_type = MUIC_CABLE_TYPE_CDP,
	 },
	{
	 .name = "USB DCP",
	 .reg_val = SM5504_DEVICE1_DCPORT,
	 .cable_type = MUIC_CABLE_TYPE_REGULAR_TA,
	 },
};
...
static const struct id_desc id_to_cable_type_mapping[] = {
	{
	 /* 00000, 0 */
	 .name = "OTG",
	 .cable_type_with_vbus = MUIC_CABLE_TYPE_OTG_WITH_VBUS,
	 .cable_type_without_vbus = MUIC_CABLE_TYPE_OTG,
	 },
	{			/* 00001, 1 */
	 .name = "AT&T TA/Unknown",
	 .cable_type_with_vbus = MUIC_CABLE_TYPE_ATT_TA,
	 .cable_type_without_vbus = MUIC_CABLE_TYPE_UNKNOWN,
	 },
	{			/* 00010, 2 */
	 .name = "AT&T TA/Unknown",
	 .cable_type_with_vbus = MUIC_CABLE_TYPE_ATT_TA,
	...
	{			/* 10100, 20 */
	 ...
	 .cable_type_with_vbus = MUIC_CABLE_TYPE_SAMSUNG_PS,
	 .cable_type_without_vbus = MUIC_CABLE_TYPE_SAMSUNG_PS,
	 },
	{			/* 10101, 21 */
	 .name = "ADC0x15 Charger/Unknown",
	 .cable_type_with_vbus = MUIC_CABLE_TYPE_0x15,
	 .cable_type_without_vbus = MUIC_CABLE_TYPE_UNKNOWN,
	 },
	{			/* 10110, 22 */
	 .name = "AT&T TA/Unknown",
	...
	{			/* 10111, 23 */
	 ...
	 .cable_type_with_vbus = MUIC_CABLE_TYPE_TYPE1_CHARGER,
	 .cable_type_without_vbus = MUIC_CABLE_TYPE_UNKNOWN,
	 },
	{			/* 11000, 24 */
	 .name = "FM BOOT OFF USB/Unknown",
	 .cable_type_with_vbus = MUIC_CABLE_TYPE_JIG_USB_OFF,
	 .cable_type_without_vbus = MUIC_CABLE_TYPE_UNKNOWN,
	 },
	{			/* 11001, 25 */
	 .name = "FM BOOT ON USB/Unknown",
	 .cable_type_with_vbus = MUIC_CABLE_TYPE_JIG_USB_ON,
	 .cable_type_without_vbus = MUIC_CABLE_TYPE_UNKNOWN,
	 },
	{			/* 11010, 26 */
	 .name = "AT&T TA/Unknown",
	...
	{			/* 11100, 28 */
	 ...
	 .cable_type_with_vbus = MUIC_CABLE_TYPE_JIG_UART_OFF_WITH_VBUS,
	 .cable_type_without_vbus = MUIC_CABLE_TYPE_JIG_UART_OFF,
	 },
	{			/* 11101, 29 */
	 .name = "JIG UART BOOT ON",
	 .cable_type_with_vbus = MUIC_CABLE_TYPE_JIG_UART_ON_WITH_VBUS,
	 .cable_type_without_vbus = MUIC_CABLE_TYPE_JIG_UART_ON,
	 },
	{			/* 11110, 30 */
	 .name = "AT&T TA/Unknown",
	 .cable_type_with_vbus = MUIC_CABLE_TYPE_ATT_TA,
	...
	{			/* 11111, 31 */
	 ...
	 .cable_type_with_vbus = MUIC_CABLE_TYPE_ATT_TA,
	 .cable_type_without_vbus = MUIC_CABLE_TYPE_NONE,
	 },
	{           /* 100000, 32 */  // MHL 0x20
	  .name = "AT&T TA/No cable",
        .cable_type_with_vbus = MUIC_CABLE_TYPE_ATT_TA,
...
static int sm5504_get_cable_type_by_id(sm5504_chip_t *chip)
{
	...
	if ( (cable_type == MUIC_CABLE_TYPE_TYPE1_CHARGER )
		...
		if (ret >= 0) {
			switch(ret) {
				...
				case 0x02 : // CDP
					...
					break;
				case 0x04 : // SDP
					cable_type = MUIC_CABLE_TYPE_L_USB;
					break;
	...
	if ((chip->prev_status.cable_type ==
		MUIC_CABLE_TYPE_OTG) && id_val == 0x1f)
		cable_type = MUIC_CABLE_TYPE_NONE;
...
static int sm5504_get_cable_type(sm5504_chip_t *chip)
{
	...
	if (chip->curr_status.ovp_status | chip->curr_status.ocp_status)
        return MUIC_CABLE_TYPE_UNKNOWN;
...
static void sm5504_preprocess_status(sm5504_chip_t *chip)
{
	...
	chip->curr_status.otg_status =
	    (chip->curr_status.cable_type ==
	     MUIC_CABLE_TYPE_OTG) ? 1 : 0;
	chip->curr_status.accessory_status =
	    ((chip->curr_status.cable_type !=
	      MUIC_CABLE_TYPE_NONE) &&
	     (chip->curr_status.cable_type !=
	      MUIC_CABLE_TYPE_UNKNOWN)) ? 1 : 0;
	...
	chip->curr_status.usb_connect =
	    ((chip->curr_status.cable_type ==
	     ...
	     (chip->curr_status.cable_type ==
	      MUIC_CABLE_TYPE_CDP) ||
	     (chip->curr_status.cable_type ==
	      MUIC_CABLE_TYPE_JIG_USB_OFF) ||
	     (chip->curr_status.cable_type ==
	      MUIC_CABLE_TYPE_L_USB) ||
	     (chip->curr_status.cable_type ==
	      MUIC_CABLE_TYPE_JIG_USB_ON)) ? 1 : 0;
	chip->curr_status.uart_connect =
	    ((chip->curr_status.cable_type ==
	      MUIC_CABLE_TYPE_UART) ||
	     (chip->curr_status.cable_type ==
	      MUIC_CABLE_TYPE_JIG_UART_OFF) ||
	     (chip->curr_status.cable_type ==
	      MUIC_CABLE_TYPE_JIG_UART_ON) ||
	     (chip->curr_status.cable_type ==
	      MUIC_CABLE_TYPE_JIG_UART_OFF_WITH_VBUS) ||
	     (chip->curr_status.cable_type ==
	      MUIC_CABLE_TYPE_JIG_UART_ON_WITH_VBUS)) ? 1 : 0;
	chip->curr_status.jig_connect =
	    ((chip->curr_status.cable_type ==
	      MUIC_CABLE_TYPE_JIG_USB_OFF) ||
	     (chip->curr_status.cable_type ==
	      MUIC_CABLE_TYPE_JIG_USB_ON) ||
	     (chip->curr_status.cable_type ==
	      MUIC_CABLE_TYPE_JIG_UART_OFF) ||
	     (chip->curr_status.cable_type ==
	      MUIC_CABLE_TYPE_JIG_UART_ON) ||
...
#if defined(CONFIG_BATTERY_SAMSUNG) || defined(CONFIG_RT_BATTERY)
	if((chip->curr_status.cable_type == MUIC_CABLE_TYPE_OTG) && (chip->curr_status.vbus_status== 1))
		cable_change_callback(MUIC_CABLE_TYPE_OTG_WITH_VBUS);
	else
#endif
	if(!chip->curr_status.ovp_status){
		if( chip->curr_status.cable_type ==MUIC_CABLE_TYPE_L_USB)
		{
		...
		if ((chip->curr_status.cable_type ==
			MUIC_CABLE_TYPE_JIG_UART_OFF_WITH_VBUS) ||
			(chip->curr_status.cable_type ==
			MUIC_CABLE_TYPE_JIG_UART_ON_WITH_VBUS)) {
...
static inline jig_type_t get_jig_type(int cable_type)
{
	...
	switch (cable_type) {
	case MUIC_CABLE_TYPE_JIG_UART_OFF_WITH_VBUS:
	case MUIC_CABLE_TYPE_JIG_UART_OFF:
		type = JIG_UART_BOOT_OFF;
		break;
	case MUIC_CABLE_TYPE_JIG_UART_ON_WITH_VBUS:
	case MUIC_CABLE_TYPE_JIG_UART_ON:
		type = JIG_UART_BOOT_ON;
		break;
	case MUIC_CABLE_TYPE_JIG_USB_OFF:
		type = JIG_USB_BOOT_OFF;
		break;
	case MUIC_CABLE_TYPE_JIG_USB_ON:
...
static void sm5504_init_regs(sm5504_chip_t *chip)
{
	...
	chip->curr_status.cable_type = MUIC_CABLE_TYPE_INVALID;
...
#endif
static DEVICE_ATTR(attached_dev, S_IRUGO, muic_show_attached_dev, NULL);
...
bool sm5504_get_vbus_status(void){
        ...
        if(NULL == chip)
                return 0;
        return chip->curr_status.vbus_status;
}
				  ...
				  const struct i2c_device_id *id)
{
	...
	if (ret < 0)
		  pr_err("Warning : failed to request GPIO%d (retval = %d)\n",
		  	pdata->irq_gpio, ret);
	ret = gpio_direction_input(pdata->irq_gpio);
	if (ret < 0)
		  pr_err("Warning : failed to set GPIO%d as input pin(retval = %d)\n",
		  	pdata->irq_gpio, ret);
	chip->iic = client;
	chip->dev = &client->dev;
	chip->pdata = pdata;
...
#elfi defined(CONFIG_MUIC_SUPPORT_FACTORY) && !defined(CONFIG_USB_SPRD_SWITCH)

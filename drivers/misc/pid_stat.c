...
static struct pid_stat *create_stat(pid_t pid)
{
	...
	create_proc_read_entry("tcp_snd", S_IRUSR, entry , tcp_snd_read_proc,
		(void *) new_sap_pid);

	create_proc_read_entry("tcp_rcv", S_IRUSR, entry, tcp_rcv_read_proc,
		(void *) new_sap_pid);
...
static int pid_stat_read_proc(char *page, char **start, off_t off,
				int count, int *eof, void *data)
{
	int len;
	unsigned long flags;
...
static int __init pid_stat_init(void)
{
	...
	pe = create_proc_read_entry("stat", S_IRUGO, parent, pid_stat_read_proc, NULL);
	if (!pe)

...
static ssize_t phy_ctl_show(struct device *pdev,
			   struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", vbus_state);
}
			    ...
			    const char *buff, size_t size)
{
	...
	if (gadget) {
		dev_info(pdev, "forcely phy_ctl by %d\n", enabled);
		if (enabled)
		...
		if (g_state == 0)
			...
			dev_err(pdev, "vbus dis/connect failed(%d)\n", g_state);
	} else {
		dev_info(pdev, "Nothing to do for phy_ctl\n");
	}

	return size;
}

static DEVICE_ATTR(phy_ctl, S_IRUGO | S_IWUSR, phy_ctl_show, phy_ctl_store);

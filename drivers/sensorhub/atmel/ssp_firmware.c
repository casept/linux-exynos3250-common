...
#define SSP_FIRMWARE_REVISION_REV00		15082700
#define SSP_FIRMWARE_REVISION			15082701
#define SSP_FIRMWARE_REVISION_FINAL		15123000
#define BL_FW_NAME			"ssp_wc1-s.fw"
#else
#define SSP_FIRMWARE_REVISION			15100100
#define BL_FW_NAME			"ssp_hwp.fw"
#endif
...
#ifdef CONFIG_MACH_WC1
	if (data->ap_rev <= 5)
		return SSP_FIRMWARE_REVISION_REV00;
	if (data->ap_rev >= 11)
		return SSP_FIRMWARE_REVISION_FINAL;
#endif
...
static int spi_write_wait_chg(struct ssp_data *data,
...
static int load_kernel_fw_bootmode(struct ssp_data *data, const char *pFn)
{
	...
	pr_info("[SSP] ssp_load_fw(%s) start!!!\n", pFn);
...
static int update_mcu_bin(struct ssp_data *data, int iBinType)
{
	...
	case KERNEL_BINARY:
		if (data->fw_name)
			iRet = load_kernel_fw_bootmode(data, data->fw_name);
		else
...
int forced_to_download_binary(struct ssp_data *data, int iBinType)
{
	...
	if (atomic_read(&data->apShutdownProgress)) {
		pr_info("[SSP]%s, AP shutdown is in progress, skip open cal data\n",
				__func__);
	} else {

		...
		struct completion *done, int timeout)
{
	...
	if (&data->comm_mutex == NULL) {
		pr_err("[SSP]%s, comm_mutex is null\n", __func__);
		return -EPERM;
	}
...
int select_irq_msg(struct ssp_data *data)
{
	...
	if (iRet < 0) {
		pr_err("[SSP] %s spi_read fail, retry count = %d, ret = %d\n",
			__func__, retry_count, iRet);
		return iRet;
	}
	...
	case AP2HUB_WRITE:
		...
		if (!list_empty(&data->pending_list)) {
			...
			if (!found) {
				pr_err("[SSP]: %s %d[%d %d %d %d] - Not match error\n",
					__func__, msg_options, chTempBuf[0],
					chTempBuf[1], chTempBuf[2],
					chTempBuf[3]);
				goto exit;
			}

			if (msg->dead && !msg->free_buffer) {
				msg->buffer = (char *) kzalloc(msg->length,
					GFP_KERNEL);
				msg->free_buffer = 1;
			...
			pr_err("[SSP]List empty error(%d, %d, %d, %d, %d)\n",
				msg_type, chTempBuf[0], chTempBuf[1],
				chTempBuf[2], chTempBuf[3]);
exit:
		mutex_unlock(&data->pending_mutex);
		break;
	case HUB2AP_WRITE:

...
int ms5837_pressure_open_calibration(struct ssp_data *data)
{
	char chBuf[10] = {0,};
	int iErr = 0;
	mm_segment_t old_fs;
...
int ms5837_set_pressure_cal(struct ssp_data *data)
{
	int iRet = 0;
	struct ssp_msg *msg;
	s32 pressure_cal;
...
void initialize_ms5837_pressure_factorytest(struct ssp_data *data)
{
	sensors_register(data->prs_device, data, pressure_attrs,
		"pressure_sensor");
}

void remove_ms5837_pressure_factorytest(struct ssp_data *data)
{

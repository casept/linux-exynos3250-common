...
static void hci_encrypt_change_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	struct hci_conn *conn;
#ifdef CONFIG_TIZEN_WIP
	u8 sec_level = SMP_AUTH_MITM | SMP_AUTH_BONDING;
#endif
...
/* Disable Secure connection implementation now */
	...
	if (ev->status && conn->state == BT_CONNECTED) {
		...
		if (conn->type == LE_LINK && ev->status == 0x06) {
			/*
			* As remote device has lost keys, so instead of disconnecting
			* remove existing keys and start pairing again.
			*/
			hci_remove_ltk(hdev, &conn->dst, conn->dst_type);

			if (test_bit(HCI_SC_ENABLED, &hdev->dev_flags))
				sec_level = sec_level | SMP_AUTH_SC;

			smp_conn_security(conn, sec_level);
		} else {
			hci_disconnect(conn, HCI_ERROR_AUTH_FAILURE);
			hci_conn_drop(conn);
			goto unlock;
...
#ifdef CONFIG_TIZEN_WIP
		struct l2cap_conn *l2cap_conn = conn->l2cap_data;

		if (l2cap_conn) {
			/* LE auto connection with BDADDR_ANY */
			bacpy(&conn->dst, &ev->bdaddr);
			conn->dst_type = ev->bdaddr_type;
			queue_work(hdev->workqueue,
					&l2cap_conn->id_addr_update_work);
		}
#endif

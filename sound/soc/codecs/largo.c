...
static int largo_get_trigger_info(struct snd_kcontrol *kcontrol,
...
SOC_SINGLE_MULTI_EXT("Trigger info", SND_SOC_NOPM, 0, 65535, 0, 3,
			largo_get_trigger_info, NULL),
};
...
/* TODO : get compress status */
int largo_get_adsp_status(struct snd_soc_codec *codec)
{
	struct largo_priv *largo = snd_soc_codec_get_drvdata(codec);

	if (largo->compr_info.stream)
		return 2; /* streaming */
	else if (largo->core.adsp[2].running)
		return 1; /* waiting */
	else
		return 0;
}
...
static void largo_dump_clk_registers(struct arizona *arizona)
{
	int ret = 0;
	unsigned int value = 0;
	int i = 0;
	unsigned int fll1_config_regs[] = {
	...
	unsigned int sysclk_ctrl_regs[] = {
				ARIZONA_SYSTEM_CLOCK_1,
				};
	unsigned int samplerate_regs[] = {
	...
	for (i = 0; i < ARRAY_SIZE(fll1_config_regs); i++) {
		ret = regmap_read(arizona->regmap, fll1_config_regs[i],
				&value);
		if (ret != 0)
			dev_err(arizona->dev,
				"Failed to get 0x%x\n", fll1_config_regs[i]);
			...
			dev_info(arizona->dev,
				"0x%x: 0x%x\n", fll1_config_regs[i], value);
	}

	for (i = 0; i < ARRAY_SIZE(fll2_config_regs); i++) {
		ret = regmap_read(arizona->regmap, fll2_config_regs[i],
				&value);
		if (ret != 0)
			dev_err(arizona->dev,
				"Failed to get 0x%x\n", fll2_config_regs[i]);
			...
			dev_info(arizona->dev,
				"0x%x: 0x%x\n", fll2_config_regs[i], value);
	}

	for (i = 0; i < ARRAY_SIZE(sysclk_ctrl_regs); i++) {
		ret = regmap_read(arizona->regmap, sysclk_ctrl_regs[i],
				&value);
		if (ret != 0)
			dev_err(arizona->dev,
				"Failed to get 0x%x\n", sysclk_ctrl_regs[i]);
			...
			dev_info(arizona->dev,
				"0x%x: 0x%x\n", sysclk_ctrl_regs[i], value);
	}

	for (i = 0; i < ARRAY_SIZE(samplerate_regs); i++) {
		ret = regmap_read(arizona->regmap, samplerate_regs[i],
				&value);
		if (ret != 0)
			dev_err(arizona->dev,
				"Failed to get 0x%x\n", samplerate_regs[i]);
			...
			dev_info(arizona->dev,
				"0x%x: 0x%x\n", samplerate_regs[i], value);
	}

}
#endif

#define DSP3_XM_BASE 0x390000
#define EZ2CTRL_WORDID_OFF 0x3D
#define SENSORY_PID_SVSCORE 0x42
#define SENSORY_PID_FINALSCORE 0x43
#define PID_NOISEFLOOR 0x47

struct sensory_status {
	unsigned int word_id;
	unsigned int sv_score;
	unsigned int final_score;
	unsigned int noise_floor;
};
static struct sensory_status sensory_value;

static int largo_get_sensory_status(struct arizona *arizona)
{
	int ret;
	unsigned int v1, v2;
	int valid_res = 0;
	...
	ret = regmap_read(arizona->regmap,
			DSP3_XM_BASE + (EZ2CTRL_WORDID_OFF * 2), &v1);
	if (ret != 0)
		dev_err(arizona->dev, "Failed to get WORD ID %d\n", ret);
	ret = regmap_read(arizona->regmap,
			DSP3_XM_BASE + (EZ2CTRL_WORDID_OFF * 2) + 1, &v2);
	if (ret != 0)
		dev_err(arizona->dev, "Failed to get WORD ID %d\n", ret);

	sensory_value.word_id = ((v1 & 0xffff) << 16) | (v2 & 0xffff);
	dev_info(arizona->dev, "WORD ID : %u\n", sensory_value.word_id);

	/* sv score */
	ret = regmap_read(arizona->regmap,
			DSP3_XM_BASE + (SENSORY_PID_SVSCORE * 2), &v1);
	if (ret != 0)
		dev_err(arizona->dev, "Failed to get sv score %d\n", ret);
	ret = regmap_read(arizona->regmap,
			DSP3_XM_BASE + (SENSORY_PID_SVSCORE * 2) + 1, &v2);
	if (ret != 0)
		dev_err(arizona->dev, "Failed to get sv score %d\n", ret);

	sensory_value.sv_score = ((v1 & 0xffff) << 16) | (v2 & 0xffff);
	dev_info(arizona->dev, "sv score : %u\n", sensory_value.sv_score);
	valid_res |= sensory_value.sv_score;

	/* final score */
	ret = regmap_read(arizona->regmap,
			DSP3_XM_BASE + (SENSORY_PID_FINALSCORE * 2), &v1);
	if (ret != 0)
		dev_err(arizona->dev, "Failed to get final score %d\n", ret);
	ret = regmap_read(arizona->regmap,
			DSP3_XM_BASE + (SENSORY_PID_FINALSCORE * 2) + 1, &v2);
	if (ret != 0)
		dev_err(arizona->dev, "Failed to get final score %d\n", ret);

	sensory_value.final_score = ((v1 & 0xffff) << 16) | (v2 & 0xffff);
	dev_info(arizona->dev, "final score : %u\n", sensory_value.final_score);
	valid_res |= sensory_value.final_score;

	/* noise floor */
	ret = regmap_read(arizona->regmap,
			DSP3_XM_BASE + (PID_NOISEFLOOR * 2), &v1);
	if (ret != 0)
		dev_err(arizona->dev, "Failed to get noise floor %d\n", ret);
	ret = regmap_read(arizona->regmap,
			DSP3_XM_BASE + (PID_NOISEFLOOR * 2) + 1, &v2);
	if (ret != 0)
		dev_err(arizona->dev, "Failed to get noise floor %d\n", ret);

	sensory_value.noise_floor = ((v1 & 0xffff) << 16) | (v2 & 0xffff);
	dev_info(arizona->dev, "noise floor : %u\n", sensory_value.noise_floor);
	valid_res |= sensory_value.noise_floor;

	return !!valid_res; /* 0 if all score is zero, others are 1 */
}

static int largo_get_trigger_info(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = sensory_value.final_score;
	ucontrol->value.integer.value[1] = sensory_value.sv_score;
	ucontrol->value.integer.value[2] = sensory_value.noise_floor;
	return 0;
}
...
static irqreturn_t adsp2_irq(int irq, void *data)
{
	...
	if (!largo->compr_info.trig &&
		...
		int res = 0;
		/* update score */
		res = largo_get_sensory_status(largo->core.arizona);
		if (!res) {
			dev_warn(largo->core.arizona->dev,
				"all score is zero. skip trigger\n");
#ifdef CONFIG_SLP_KERNEL_ENG
			largo_dump_clk_registers(largo->core.arizona);
#endif
		}
		if (largo->core.arizona->pdata.ez2ctrl_trigger && res)
...
static int largo_free(struct snd_compr_stream *stream)
{
	...
	dev_info(largo->core.arizona->dev, "%s\n", __func__);
	mutex_unlock(&largo->compr_info.lock);

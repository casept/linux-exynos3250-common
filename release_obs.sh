...
echo "defconfig : ${CONFIG_STR}_defconfig , option : ${OPTION_STR}, Release : ${RELEASE}, Retail : ${RETAIL}"
...
if [ "${RETAIL}" = "ldu" ] && [ "${MODEL}" = "tizen_volt" ]; then
	echo "Now enable Retail config for ${CONFIG_STR}_defconfig"

	sed -i 's/\# CONFIG_RETAIL_DEMO is not set/CONFIG_RETAIL_DEMO=y/g' arch/arm/configs/${CONFIG_STR}_defconfig
	if [ "$?" != "0" ]; then
		echo "Failed to enable CONFIG_RETAIL_DEMO feature"
		exit 1
	fi
fi

...
int pm_slave_wake_lock(const char *buf)
{
	...
	pr_debug("%s: wl=%s: slave_wl=%s: %d: %lu: %lld\n",
			__func__, wl->name, slave_wl->name, slave_wl->active,
			slave_wl->active_count, ktime_to_ms(slave_wl->prevent_time));
...
int pm_slave_wake_unlock(const char *buf)
{
	...
	if (slave_wl->active) {
		...
		if (wl->ws.autosleep_enabled) {
			...
			if (ktime_equal(slave_wl->prevent_time, zero))
				slave_wl->prevent_time = delta;
			else
				slave_wl->prevent_time =  ktime_add(slave_wl->prevent_time, delta);
		}
	}
	pr_debug("%s: wl=%s: slave_wl=%s: %d: %lu: %lld\n",
			__func__, wl->name, slave_wl->name, slave_wl->active,
			...
			struct slave_wakelock **last_slave_wl_array, int slave_wl_count)
{
	...
	struct wakelock *wl;
	struct slave_wakelock *slave_wl, *long_activity_wl = NULL;
	...
	list_for_each_entry(slave_wl, &wl->slave_wl_list, node) {
		if (slave_wl->active && active_count < slave_wl_count)
	...
	if (active_count < slave_wl_count)
		list_for_each_entry(slave_wl, &wl->slave_wl_list, node) {
 			if (!slave_wl->active && !(ktime_equal(slave_wl->prevent_time, zero))) {
				if (active_count < slave_wl_count)
					*(&last_slave_wl_array[array_count++]) = slave_wl;
				else {
					long_activity_wl = slave_wl;
					for ( i = active_count; i < slave_wl_count; i++) {
						if (ktime_to_ns(long_activity_wl->prevent_time) >
							ktime_to_ns((*(&last_slave_wl_array[i]))->prevent_time))
							swap(*(&last_slave_wl_array[i]), long_activity_wl);
					}
				}
			}
		} 
...
int pm_del_slave_wakelock_prevent_sleep_time(struct wakeup_source *ws)
{
	int ret = -1;
	struct wakelock *wl;
	struct slave_wakelock *slave_wl;
	...
	list_for_each_entry(slave_wl, &wl->slave_wl_list, node) {
		if (!slave_wl->active)
			slave_wl->prevent_time  =  ktime_set(0, 0);
	}
...
static int slave_wakelocks_stats_show(struct seq_file *m, void *unused)
{
	...
	seq_puts(m, "m-name\t\ts-name\t\tactive_count\t"
		"active_since\ttotal_time\tmax_time\tmax_time_stamp\t"
	...
	for (node = rb_first(&wakelocks_tree); node; node = rb_next(node)) {
		...
		list_for_each_entry(slave_wl, &wl->slave_wl_list, node) {
			...
			max_time_stamp = slave_wl->max_time_stamp;
			...
			if (slave_wl->active) {
				...
				if (active_time.tv64 > max_time.tv64) {
					...
					max_time_stamp = slave_wl->last_time;
				}
			...
			seq_printf(m, "%-12s\t%-12s\t%lu\t\t"
				...
				ktime_to_ms(slave_wl->last_time),
